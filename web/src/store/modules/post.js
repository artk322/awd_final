...
export default {
  ...
  getters: {
    ...
    owner_phoner_number: state => state.owner_phoner_number,
  ...
  mutations: {
    ...
    setPhoneNumber(state, phone_number) {
      state.owner_phoner_number = phone_number;
    },
    clearPhoneNumber(state) {
      state.owner_phoner_number = '';
    },
  },
  actions: {
    ...
    async fetchOwnerPhoneNumber({ commit }, payload) {
      await axios
        .post(api + 'fetchOwnerPhoneNumber', payload)
        .then(response => {
          commit('setPhoneNumber', response.data);
        })
        .catch(error => {
          commit('setMessage', { type: 'error', message: error.response.data.message });
        });
