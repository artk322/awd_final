    ...
    <div class="container" style="padding-right: 10px; padding-left:10px;">
      ...
      <div class="row">
        <div class="col-12 gallery">
          <tiny-slider :mouse-drag="true" :loop="false" :nav="false" :controls-text="['', '']" :gutter="20">
            <img
      ...
      <div v-else>
        <hr />
        <h5>
            Номер автора объявления:
            <span v-if="owner_phoner_number">{{ owner_phoner_number }}</span>
            <!-- <SubmitButton
              v-else
              :loading="loading_phone"
              buttonLabel="Показать номер телефона"
              buttonClass="btn btn-primary"
              @click.native="fetchOwnerPhoneNumber"
            /> -->
            <a v-if="!loading_phone && !owner_phoner_number" href class="link" @click.prevent="fetchOwnerPhoneNumber">Показать</a>
            <span v-if="loading_phone">
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['sidebarActive', 'userData', 'owner_phoner_number']),
  },
  async mounted() {
    this.pid = +this.$route.params.id;
    await this.$store.dispatch('fetchPostByPid', { pid: this.pid });
    this.loading = false;
  },
  beforeDestroy() {
  ...
  methods: {
    async fetchOwnerPhoneNumber() {
      this.loading_phone = true;
      await this.$store.dispatch('fetchOwnerPhoneNumber', { pid: this.pid })
