...
const fetchPostByPid = async (req, res) => {
  ...
  const { rows: [post] } = await pool.query('SELECT pid, title, price, location, space, images, categories, description, owner_uid, status, features_id FROM posts WHERE pid = $1', [pid]);
...
const fetchOwnerPhoneNumber = async (req, res) => {
  const { pid } = req.body;

  try {
    const { rows: [{owner_uid}] } = await pool.query('SELECT owner_uid FROM posts WHERE pid = $1', [pid]);
    const { rows: [{phone_number}] } = await pool.query('SELECT phone_number FROM users WHERE uid = $1', [owner_uid]);
      ...
      errorPost => {
        ...
        features.map(it => {
          ...
          pool.query(
            ...
            [geoJSON, gid, pid],
            errorGeom => {
              if (errorGeom) geom_errors.push(errorGeom);
            }
          );
        });
